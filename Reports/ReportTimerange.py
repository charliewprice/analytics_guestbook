...
def mmyyyy(month, year):
  now = dtime.now()
  START_DATE = dtime.combine(now.replace(year=year, month=month, day=1), time.min)
  END_DATE = dtime.combine(now.replace(year=year, month=month, day=31), time.max)
